...
def main(
        ...
        prompts = ["a side view photo of a a man standing", "a side view photo of a a man running"], # prompts to dream about
        seeds=[243, 523],
        ...
        num_inference_steps = 50, # number of inference steps per dream
        guidance_scale = 7.5,
        eta = 0.0,
        width = 512,
        height = 512,
    ...
    for p, prompt_embedding_b in enumerate(prompt_embeddings):
        ...
        for i, t in enumerate(np.linspace(0, 1, num_steps)):
            ...
            # im.save(outpath)
            if(frame_index == 0):
                video_file = cv2.VideoWriter(rootdir + "/" + video_name + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), 30, frameSize=im.size)   
